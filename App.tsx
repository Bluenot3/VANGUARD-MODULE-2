...
const App: React.FC = () => {
    ...
    const { user, updateProgress, isResetting } = useAuth();
    ...
    const lastSectionId = useMemo(() => {
        const findLast = (sections: Section[]): string => {
            const last = sections[sections.length - 1];
            return last.subSections && last.subSections.length > 0
                ? findLast(last.subSections)
                : last.id;
        };
        return findLast(curriculumData.sections);
    }, []);
            ...
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        if(!isResetting && user && !user.progress.completedSections.includes(entry.target.id)) {
                           updateProgress(entry.target.id, 'section');
                           if (entry.target.id === lastSectionId) {
    ...
    }, [allSections, updateProgress, user, lastSectionId, isResetting]);
